...
def precompute_dataset(dataset_path, cache_dir='pose_cache_lstm'):
    """
    Scans folders, runs YOLO, and caches the LSTM-ready tensors.
    """
    os.makedirs(cache_dir, exist_ok=True)
    yolo_model = YOLO('yolo11n-pose.pt') # Use 'n' for speed, 'm' for accuracy
...
def main():
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        if acc > best_acc:
            best_acc = acc
            torch.save(model.state_dict(), 'best_violence_lstm.pt')
            print("--> Model Saved!")
